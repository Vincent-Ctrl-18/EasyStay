...
export default function HotelForm() {
  ...
  useEffect(() => {
    const loadHotelData = async () => {
      ...
      console.log('Loading hotel with id:', id); // 添加日志
      setLoading(true);
      
      try {
        // 直接测试后端 API，跳过代理
        const response = await fetch(`http://localhost:3001/api/hotels/${id}`);
        console.log('Fetch response status:', response.status); // 添加日志
        ...
        console.log('Hotel data:', data); // 添加日志
        ...
        console.error('Error loading hotel:', error); // 添加错误日志
        message.error('加载酒店信息失败'); // 直接使用字符串，避免依赖 t
      } finally {
        console.log('Loading completed'); // 添加日志
        setLoading(false);
      }
    };
    
    loadHotelData();
  }, [id, isEdit]); // 只依赖 id 和 isEdit，避免无限循环
