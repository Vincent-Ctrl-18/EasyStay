...
export default function HotelList() {
  ...
  const handleApplyOnline = async (id: number) => {
    try {
      await hotelAPI.applyOnline(id);
      message.success('已提交恢复上线申请');
      fetchHotels();
    } catch (err: any) {
      ...
      render: (_: any, record: any) => (
        <Space>
          ...
          {record.status === 'offline' && (
            <Popconfirm title="确定申请恢复上线吗？" onConfirm={() => handleApplyOnline(record.id)}>
